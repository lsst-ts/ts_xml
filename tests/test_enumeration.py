...
from lsst.ts.xml import enums, get_xmlfile_csc_topic, subsystems, subsystems_to_enums
...
def test_enumeration(xmlfile: pathlib.Path, csc: str, topic: str) -> None:
    ...
    for sal_enum in root.findall("Enumeration"):
        ...
        for enum_value in sal_enum.text.split(","):
            assert reg_exp.match(enum_value.strip()) is not None, (
...
def test_enumeration_consistency(xmlfile: pathlib.Path, csc: str, topic: str) -> None:
    ...
    for sal_enum in root.findall("Enumeration"):
        ...
        assert num_lines_with_explicit_value == 0 or num_lines_without_explicit_value == 0, (
...
def test_enum_classes(csc: str) -> None:
    ...
    enum_for_csc = subsystems_to_enums[csc]

    assert hasattr(enums, enum_for_csc), (
        f"{csc} does not define enumeration. Expected {global_enums_set} enums."
    )
    assert hasattr(getattr(enums, enum_for_csc), "__all__"), (
        f"{enum_for_csc} enumeration does not define __all__."
    )

    try:
        enums_set = set(getattr(enums, enum_for_csc).__all__)
    except AttributeError:
        raise RuntimeError(f"{enum_for_csc} does not define a valid enumeration.")

    missing_enums = global_enums_set.difference(enums_set)
    undefined_enums = enums_set.difference(global_enums_set) - {"SalIndex"}

    assert not missing_enums, f"[{csc}] The following enums are missing: {missing_enums}."
    assert not undefined_enums, (
